    ...
    <div class="show-hero-content">
        ...
        <div class="show-hero-meta" style="margin-top:.75rem">
            ...
            @foreach(array_slice($game['genres'] ?? [], 0, 3) as $genre)
                <span class="esrb-badge" style="background:rgba(255,255,255,.08);border-color:rgba(255,255,255,.15);color:rgba(255,255,255,.7)">
                    {{ $genre['name'] }}
                </span>
            @endforeach
        </div>
    ...
    <div class="show-main">
        ...
        <div class="cgame-toolbar">
            <div class="cgame-toolbar-left">
                <span class="cgame-count">
                    @if($rulesets->total() > 0)
                        <strong style="color:var(--text)">{{ $rulesets->total() }}</strong> {{ Str::plural('ruleset', $rulesets->total()) }} found
                    @else
            ...
            @auth
                <a href="{{ route('rulesets.create', ['game_id' => $game['id'], 'game_name' => $game['name'], 'game_image' => $game['background_image'] ?? '']) }}" class="btn btn-primary btn-sm cgame-create-btn">
                    ...
                    <a href="{{ route('rulesets.show', $ruleset) }}" class="cgame-ruleset-card">
                            ...
                            <div class="cgame-ruleset-rules-preview">
                                ...
                                @if(count($ruleset->rules) > 3)
                                    <div class="cgame-ruleset-rule-more">
                                        +{{ count($ruleset->rules) - 3 }} more {{ Str::plural('rule', count($ruleset->rules) - 3) }}
                                    </div>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="cgame-ruleset-footer">
                            ...
                            <div class="cgame-ruleset-meta-right">
                                ...
                                <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="color:var(--muted)"><path d="M9 18l6-6-6-6"/></svg>
                            </div>
                        </div>
        ...
        @else
            ...
            <div class="cgame-empty">
                ...
                <p class="cgame-empty-sub">Be the first to create a challenge ruleset for <strong>{{ $game['name'] }}</strong>.</p>
                @auth
                    <a href="{{ route('rulesets.create', ['game_id' => $game['id'], 'game_name' => $game['name'], 'game_image' => $game['background_image'] ?? '']) }}" class="btn btn-primary" style="margin-top:1.5rem">
                ...
                @else
                    <a href="{{ route('login') }}" class="btn btn-ghost" style="margin-top:1.5rem">Sign in to create a ruleset</a>
                @endauth
            </div>
        @endif
    ...
    <div class="show-sidebar">
        ...
        <div class="show-card" style="padding:0;overflow:hidden">
            ...
            <div style="padding:1.25rem 1.5rem">
                ...
                <p style="font-family:'Syne',sans-serif;font-weight:700;font-size:1rem;line-height:1.3;margin-bottom:1rem">{{ $game['name'] }}</p>
                ...
                <a href="{{ route('games.show', $game['id']) }}" class="btn btn-ghost btn-sm" style="width:100%;text-align:center;display:block;margin-top:1rem">
                    View full game page
                </a>
            </div>
        </div>
        ...
        @auth
            <div class="show-card cgame-cta-card">
                ...
                <a href="{{ route('rulesets.create', ['game_id' => $game['id'], 'game_name' => $game['name'], 'game_image' => $game['background_image'] ?? '']) }}" class="btn btn-primary btn-sm cgame-cta-btn">
                    <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                    Create a ruleset
                </a>
            </div>
